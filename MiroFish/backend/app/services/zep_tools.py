...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### 【Current Active Facts】(Simulation result original text)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (split at period, not hard truncation)
...
class ZepToolsService:
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all edges in the graph, then filters for edges related to the specified node
...
Return the sub-question list in JSON format."""
    ...
    def interview_agents(
        ...
        # Step 4: Call the real interview API (no platform specified, defaults to dual-platform simultaneous interview)
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                twitter_text = twitter_response if twitter_response else "(No response obtained from this platform)"
                reddit_text = reddit_response if reddit_response else "(No response obtained from this platform)"
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        """Load simulated agent profile files"""
        import os
        import csv
...
5. Each question should be concise and clear, under 50 words
...
4. Remain objective and neutral, not favoring any party
