...
Converts entities from the Zep graph into Agent Profile format required by the OASIS simulation platform
...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search API, so edges and nodes must be searched
        separately and results merged. Uses parallel requests to improve efficiency.
    ...
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if this is a group/institutional entity type"""
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newline characters within the string with spaces
                s = s.replace('\n', ' ').replace('\r', ' ')
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user profiles. Generate detailed, realistic personas for opinion simulation, restoring existing real-world situations as closely as possible. You must return valid JSON format, and all string values must not contain unescaped newline characters. Use Chinese."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, restoring existing real-world situations as closely as possible.
...
2. persona: Detailed persona description (2000 characters of plain text), should include:
   ...
   - Positions and views (attitudes toward topics, content that may provoke/move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memories (an important part of the persona, introduce this individual's connection to events, and their existing actions and reactions in those events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, restoring existing real-world situations as closely as possible.
...
2. persona: Detailed account profile description (2000 characters of plain text), should include:
   ...
   - Communication style (language characteristics, commonly used expressions, taboo topics)
   ...
   - Institutional memories (an important part of the institutional persona, introduce this institution's connection to events, and its existing actions and reactions in those events)
    ...
    def generate_profiles_from_entities(
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Output the generated persona to console and logs in real time
